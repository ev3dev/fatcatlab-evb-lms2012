...
#include "vol_char_8x20.txt"
#include "logo_41x54.txt"
#include "fatcatlab_178x24.txt"
...
void InitGpio(void)
{
	...
	for (Pin = 0; Pin < NO_OF_BUTTONS; Pin++)
	{
		...
		{
			if((UiButPin[Pin].Pin) >= 0 && (UiButPin[Pin].Pin) < 128)
			{
				if ((UiButPin[Pin].Pin) < 32)
				{
					UiButPin[Pin].pGpio  =  GPIOBANK0;
				}
				else if ((UiButPin[Pin].Pin) < 64)
				{
					UiButPin[Pin].pGpio  =  GPIOBANK1;
				}
				else if ((UiButPin[Pin].Pin) < 96)
				{
					UiButPin[Pin].pGpio  =  GPIOBANK2;
				}
				else
				{
					UiButPin[Pin].pGpio  =  GPIOBANK3;
				}
...
void lcd_put_vol_char(int x, int y, unsigned char letter, unsigned char c0, unsigned char c1)
{
	...
	for (line = 0; line < 30; line++)
	{
		do
		{
			byte = vol_char_16x30[base + line * 2 + n];
			for (bit = 7; bit >= 0; bit--)
			{
				if(byte & (1 << bit))
				{
					lcd_put_pixel(x + 7 - bit + n * 8, y + line, c0, c1);
				}
				else
				{
					lcd_put_pixel(x + 7 - bit + n * 8, y + line, 0, 0);
				}
			}
		}
		while(++n < 2);
...
//	GetPeriphealBasePtr(0x44E00000, 0x154, (ULONG **)&CM_PER);
	GetPeriphealBasePtr(0x44E00400, 0xDC, (ULONG **)&CM_WKUP);
