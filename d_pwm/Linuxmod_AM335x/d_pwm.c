...
//Changed for BBB: The clock of pwm module is 100MHz in BBB other than 132MHz in EV3, so this parameter shoule be changed to 7600
 
#define   MAX_PWM_CNT                   (7600)
...
//Changed for BBB: The clock of timer is 24MHz in BBB other than 33MHz in EV3, so these parameters shoule be changed, see line 3726
...
// Timer Registers
#define	TIDR		0x0
#define TIOCP_CFG	(0x10 >> 2)
#define IRQ_EOI		(0X20 >> 2)
#define IRQ_STATUS_RAW	(0x24 >> 2)
#define	IRQSTATUS	(0x28 >> 2)
#define	IRQENABLE_SET	(0x2C >> 2)
#define	IRQENABLE_CLR	(0x30 >> 2)
#define	IRQWAKEEN	(0x34 >> 2)
#define	TCLR		(0x38 >> 2)
#define	TCRR		(0x3C >> 2)
#define	TLDR		(0x40 >> 2)
#define	TTGR		(0x44 >> 2)
...
{
	[EP2]		=	(INPIN*)&EP2_OutputPortPin[0],
};


#define OutputReadDir(port,pin)		((HwInvBits ^ ((pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].pGpio)[GPIO_DATAIN])) & pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].Mask)
...
static volatile ULONG *PWMSS1;
static volatile ULONG *PWMSS2;

static volatile UWORD *eHRPWM1;
static volatile UWORD *eHRPWM2;

static ULONG *DMTIMER3;		//It's important not to add the "volatile"!
...
#define   FLOATFaultPins                {\
		...
		{\
			(pOutputPortPin[Hw][(0 * OUTPUT_PORT_PINS) + FAULT].pGpio)[GPIO_OE] |= pOutputPortPin[Hw][(0 * OUTPUT_PORT_PINS) + FAULT].Mask;\
		}\
		if (((pOutputPortPin[Hw])[(2 * OUTPUT_PORT_PINS) + FAULT].Pin) != -1)\
		{\
			(pOutputPortPin[Hw][(2 * OUTPUT_PORT_PINS) + FAULT].pGpio)[GPIO_OE] |= pOutputPortPin[Hw][(2 * OUTPUT_PORT_PINS) + FAULT].Mask;\
		}\
	}
...
#define   SETSleepPins                  {\
		...
		{\
			(pOutputPortPin[Hw][(0 * OUTPUT_PORT_PINS) + SLEEP].pGpio)[GPIO_SETDATAOUT] = pOutputPortPin[Hw][(0 * OUTPUT_PORT_PINS) + SLEEP].Mask;\
			(pOutputPortPin[Hw][(0 * OUTPUT_PORT_PINS) + SLEEP].pGpio)[GPIO_OE] &= ~pOutputPortPin[Hw][(0 * OUTPUT_PORT_PINS) + SLEEP].Mask;\
		}\
		if (((pOutputPortPin[Hw])[(2 * OUTPUT_PORT_PINS) + SLEEP].Pin) != -1)\
		{\
			(pOutputPortPin[Hw][(2 * OUTPUT_PORT_PINS) + SLEEP].pGpio)[GPIO_SETDATAOUT] = pOutputPortPin[Hw][(2 * OUTPUT_PORT_PINS) + SLEEP].Mask;\
			(pOutputPortPin[Hw][(2 * OUTPUT_PORT_PINS) + SLEEP].pGpio)[GPIO_OE] &= ~pOutputPortPin[Hw][(2 * OUTPUT_PORT_PINS) + SLEEP].Mask;\
		}\
	}
...
#define   FREERunning24bittimer         ((ULONG)((((ULONG*)(DMTIMER3))[TCRR])>>8))
...
#define   OutputFloat(port,pin)         {\
                                          (pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].pGpio)[GPIO_OE] |=  pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].Mask;\
                                        }

#define   OutputRead(port,pin)          ((pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].pGpio)[GPIO_DATAIN] & pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].Mask)

#define   OutputHigh(port,pin)          {\
                                          (pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].pGpio)[GPIO_SETDATAOUT]  =  pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].Mask;\
                                          (pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].pGpio)[GPIO_OE]      &= ~pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].Mask;\
                                        }

#define   OutputLow(port,pin)           {\
                                          (pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].pGpio)[GPIO_CLEARDATAOUT]  =  pOutputPortPin[Hw][(OUTPUT_PORT_PINS * port) + pin].Mask;\
                                          (pOutputPortPin[Hw][(port * OUTPUT_PORT_PINS) + pin].pGpio)[GPIO_OE]      &= ~pOutputPortPin[Hw][(OUTPUT_PORT_PINS * port) + pin].Mask;\
                                        }

#define EHRPWMClkDis	{\
		eHRPWM2[TBCTL] = (TB_UP | TB_DISABLE | TB_SHADOW | TB_SYNC_DISABLE | TB_HDIV1 | TB_DIV1 | TB_COUNT_FREEZE);\
...
#define EHRPWMClkEna	{\
		PWMSS2[0x2] |= 0x100;\
		PWMSS1[0x2] |= 0x100;\
		eHRPWM2[TBCTL] = (TB_UP | TB_DISABLE | TB_SHADOW | TB_SYNC_DISABLE | TB_HDIV1 | TB_DIV1 | TB_COUNT_UP);\
		eHRPWM1[TBCTL] = (TB_UP | TB_DISABLE | TB_SHADOW | TB_SYNC_DISABLE | TB_HDIV1 | TB_DIV1 | TB_COUNT_UP);\
		CM[0x199] |= 0x6;/* Timebase for PWMSS1 & PWMSS2 enable */\
}

#define STOPPwm		{\
		iowrite16(0x00, &eHRPWM2[TBCTL]);\
		iowrite16(0x00, &eHRPWM2[CMPCTL]);\
...
#define SETUPPwmModules		{\
		...
		eHRPWM2[TBPHS] = 0;\
		eHRPWM2[TBCNT] = 0;\
		eHRPWM2[CMPCTL] =  (CC_A_SHADOW | CC_B_SHADOW | CC_CTR_A_ZERO | CC_CTR_B_ZERO);\
		eHRPWM2[AQCTLA] = 0x21;\
		eHRPWM2[AQCTLB] = 0x201;\
		eHRPWM1[TBPHS] = 0;\
		eHRPWM1[TBCNT] = 0;\
		eHRPWM1[CMPCTL] =  (CC_A_SHADOW | CC_B_SHADOW | CC_CTR_A_ZERO | CC_CTR_B_ZERO);\
		eHRPWM1[AQCTLA] = 0x21;\
		eHRPWM1[AQCTLB] = 0x201;\
		EHRPWMClkEna;\
		DMTIMER3[TMAR] = 0xFFFFFFFF;\
		DMTIMER3[TCLR] = 0x2;\
		DMTIMER3[TCLR] |= 0x1;\
		\
		SetDutyMA(0);\
...
void SetDutyMA(ULONG Duty)
{
	eHRPWM2[CMPB] = (UWORD)Duty;
}
void SetDutyMB(ULONG Duty)
{
	eHRPWM2[CMPA] = (UWORD)Duty;
}
void SetDutyMC(ULONG Duty)
{
...
static int Device1Init(void)
{
	...
	GetPeriphealBasePtr(0x48302000, 0x10, (ULONG **)&PWMSS1);
	GetPeriphealBasePtr(0x48302200, 0x44, (ULONG **)&eHRPWM1);
	GetPeriphealBasePtr(0x48304000, 0x10, (ULONG **)&PWMSS2);
	GetPeriphealBasePtr(0x48304200, 0x44, (ULONG **)&eHRPWM2);
	GetPeriphealBasePtr(0x48042000, 0x5B, (ULONG **)&DMTIMER3);

	Result = misc_register(&Device1);


	for(Tmp = 0; Tmp < NO_OF_OUTPUT_PORTS; Tmp++)                 //fen bie chu shi hua 4 ge shu chu duan kou
	{
...
void InitGpio(void)
{
	...
	for (Port = 0; Port < NO_OF_OUTPUT_PORTS; Port++)
	{
		...
		{
			if((pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].Pin) >= 0 && (pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].Pin) < 128)
			{
				if ((pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].Pin) < 32)
				{
					pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].pGpio  =  GPIOBANK0;
				}
				else if ((pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].Pin) < 64)
				{
					pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].pGpio  =  GPIOBANK1;
				}
				else if ((pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].Pin) < 96)
				{
					pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].pGpio  =  GPIOBANK2;
				}
				else
				{
					pOutputPortPin[Hw][(Port * OUTPUT_PORT_PINS) + Pin].pGpio  =  GPIOBANK3;
				}
...
void    SetGpioRisingIrq(UBYTE PinNo, irqreturn_t (*IntFuncPtr)(int, void *))
{
  ...
  irq_set_irq_type(gpio_to_irq(PinNo), IRQ_TYPE_EDGE_RISING | IRQ_TYPE_EDGE_FALLING);
}
...
static int ModuleInit(void)
{
	...
	CM_PER[0xCC >> 2] |= 0x2;	//CM_PER_EPWMSS1_CLKCTRL
	while(0x2 != (ioread32(&CM_PER[0xCC >> 2]) & 0x2))
		;
	CM_PER[0xD8 >> 2] |= 0x2;	//CM_PER_EPWMSS2_CLKCTRL
	while(0x2 != (ioread32(&CM_PER[0xD8 >> 2]) & 0x2))
		;
	CM_PER[0x84 >> 2] |= 0x2;	//CM_PER_TIMER3_CLKCTRL
	while(0x2 != (ioread32(&CM_PER[0x84 >> 2]) & 0x2))
		;
